    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-xs-sm max-w-3xl w-full">
      <div class="p-6">
        ...
        <p class="text-sm text-gray-500 mt-1">
          {{ partenaire ? 'Modifiez les informations du partenaire' : 'Créez un nouveau partenaire' }}
        </p>
      </div>
      ...
      <form @submit.prevent="submit" class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="md:col-span-2">
          ...
          <input
            v-model="form.nom"
            type="text"
            placeholder="Croissant-Rouge Sahélien, MSF..."
        ...
        <div>
          <label class="block text-xs font-medium text-gray-600 mb-1">Projet associé</label>
          <input
            v-model="form.projet"
            type="text"
      ...
      <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3">
        ...
        <BoutonPrimary @click="submit">
          {{ partenaire ? 'Mettre à jour' : 'Enregistrer' }}
        </BoutonPrimary>
      </div>
    </div>
  </div>
